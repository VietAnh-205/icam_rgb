...
stage1_batch_size = 128 # batch size for training in stage 1
...
logger_params = {
    'log_file': {
        'desc': f"icam_cvrptw{min_problem_size}_to_{max_problem_size}_epoch{training_epochs}",
        'filename': 'run_log.txt',
        'filepath': f'./result_cvrptw_models/' + process_start_time.strftime("%Y%m%d_%H%M%S") + '{desc}'
    }
}
...
def main():
    ...
    copy_all_src(trainer.result_folder) # if python version >= 3.9, please remove this line
    trainer.run()
...
def debug_mode():
    training_epochs = 5 # total epochs for training
    batches_per_epoch = 5 # steps per epoch
    stage1_epochs = 2 # epochs for stage 1
    stage3_epochs = 2 # epochs for stage 3
    
    trainer_params['epochs'] = training_epochs
    trainer_params['stage1_epochs'] = stage1_epochs
    trainer_params['batches_per_epoch'] = batches_per_epoch
    optimizer_params['lr_decay_epoch'] = training_epochs - stage3_epochs + 1
