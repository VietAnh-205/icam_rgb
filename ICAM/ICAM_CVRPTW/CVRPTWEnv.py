...
class CVRPTWEnv:
    ...
    def load_problems_cvrptw(self, batch_size, problem_size,capacity=None,pomo_size=None,validation_data=None, aug_factor=1, device=None):
        if validation_data is not None:
            depot_xy, node_xy, node_demand, service_time, tw_start, tw_end = validation_data
        else:
            depot_xy, node_xy, node_demand, service_time, tw_start, tw_end = self.get_random_problems(batch_size, problem_size, demand_scaler=capacity,normalized=True)
        ...
        if node_demand.max() > 1:
            raise ValueError("Node demand should be normalized to [0, 1].")
    ...
    def step(self, selected):
        ...
        # shape: (batch, pomo)
        # the below is used for CVRP with duration limit constraint
        ...
        out_of_tw = arrival_time > self.depot_node_tw_end[:, None, :].expand(-1, self.pomo_size, -1) + self.round_error_epsilon
        # shape: (batch, pomo, problem+1)
        self.ninf_mask[out_of_tw] = float('-inf')
        fail_return_depot = arrival_time + self.depot_node_service_time[:, None, :].expand(-1, self.pomo_size, -1) + (self.depot_node_xy[:, None, :1, :] - self.depot_node_xy[:, None, :, :].expand(-1, self.pomo_size, -1, -1)).norm(p=2, dim=-1) / self.speed > self.depot_end + self.round_error_epsilon
        # shape: (batch, pomo, problem+1)
        ...
        self.step_state.finished = self.finished
        self.step_state.current_time = self.current_time
        # self.step_state.length = self.length # used for CVRP with duration limit constraint
        self.step_state.current_coord = self.current_coord
    ...
    def generate_dataset(self, num_samples, problem_size, demand_scaler, path):
        data = self.get_random_problems(num_samples, problem_size, demand_scaler=demand_scaler, normalized=False)
    ...
    def load_dataset(self, path, offset=0, num_samples=1000, disable_print=True,solution_name=None):
        ...
        if node_demand.max() <= 1:
            raise ValueError("The loaded demand seems to be normalized, please set 'normalized=False' when generating the dataset.")
        ...
        if solution_name is not None:
            with open(solution_name, 'rb') as f2:
                out_2 = pickle.load(f2)[offset: offset+num_samples]
                out_2 = np.array(out_2, dtype=object)[:, 0].tolist()
                optimal_score_all = torch.tensor(out_2, dtype=torch.float32)
                optimal_score = optimal_score_all.mean().item()
            return data, optimal_score,capacity[0]
        else:
            return data

    def get_random_problems(self, batch_size, problem_size, demand_scaler, normalized=True):
        ...
        if instance_invalid:
            ...
            return self.get_random_problems(batch_size, problem_size, demand_scaler=demand_scaler, normalized=normalized)
        elif normalized:
