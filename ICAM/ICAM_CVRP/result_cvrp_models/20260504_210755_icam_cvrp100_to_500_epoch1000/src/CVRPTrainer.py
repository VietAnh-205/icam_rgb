...
class CVRPTrainer:
    ...
    def _train_one_epoch(self, epoch):
        ...
        # Thêm vào cuối _train_one_epoch() trong CVRPTrainer.py
        import torch.nn.functional as F
        for i, layer in enumerate(self.model.encoder.layers):
            self.logger.info(f"  Encoder layer {i} γ = {F.softplus(layer.gamma).item():.4f}")
    ...
    def _validation_one_epoch(self, dataset, optimal_score, epoch):
        ...
        batch_size = dataset['node_xy'].shape[0]
        problem_size = dataset['node_xy'].shape[1]
        with torch.no_grad():
            self.env.load_problems_cvrp(batch_size=batch_size,
                                       problem_size=problem_size,
