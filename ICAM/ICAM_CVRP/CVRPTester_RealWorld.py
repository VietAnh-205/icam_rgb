...
from CVRPEnv_RealWorld import CVRPEnv as Env
...
class CVRPTester:
    ...
    def run(self):
        ...
        if self.tester_params['test_data_load']['enable']:
            ...
            self.env.input_saved_data(self.tester_params['test_data_load']['filename'], 
                                      test_num_episode, 
                                      problem_size, 
                                      self.device, 
                                      solution_name=self.tester_params['test_data_load']['solution_filename'])
            optimal_score = self.env.optimal_score
            self.logger.info("Saved dataset loaded successfully!!!")
            self.logger.info("Data loaded from: {0}".format(self.tester_params['test_data_load']['filename']))
            self.logger.info('problem_size: {0}, capacity: {1}, optimal: {2:.4f}'.format(
                self.env.saved_node_xy.shape[1], self.env.capacity, optimal_score))
        ...
        self.logger.info(f"start solve {self.env_params['city']},batch_size={self.tester_params['test_batch_size']},problem_size={self.env_params['problem_size']},capacity={self.env.capacity}")
        ...
        while episode < test_num_episode:
            ...
            if all_done:
                ...
                self.logger.info("===============================================================")
                self.logger.info(" problem size: {0}, pomo size: {1}, city: {2}, optimal score: {3:.4f} ".format(
                    self.env_params['problem_size'], self.env_params['pomo_size'], self.env_params['city'],optimal_score))
                ...
                self.logger.info(" Total time: {:.3f} sec".format(end_time - start_time))
                self.logger.info(" Avg time per episode: {:.4f} sec".format((end_time - start_time) / test_num_episode))
    ...
    def _test_one_batch(self, batch_size):
        ...
        self.model.eval()
        with torch.no_grad():
            self.env.load_problems_cvrp_real_world(batch_size,
                                        problem_size=self.env_params['problem_size'],
